...
def run_cmd(cmd, ignore_error=False, verbose=False):
    ...
    logging.debug('%s' %cmd)
    try: proc = subprocess.Popen(cmd,shell=(sys.platform!="win32"),
                            stdin=subprocess.PIPE,stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE)
    except OSError,e:
        if not ignore_errot: logger.error('Failed at running %s!\n%s' %(cmd,e.child_traceback))
        return False
    out = proc.communicate()
    return_code = proc.returncode
    if return_code != 0 and not ignore_error:
        logging.error('Command (%s) failed w/ error %d\n\n\x1b[31m%s\x1b[0m\n' %(cmd, return_code,str(out[1].decode('utf-8'))))
        sys.exit()
...
def adjustNodes(G,oriDicts):
	...
	for oriDict in oriDicts:
		for n,v in oriDict.iteritems(): G.node[n]['orientation'] =  G.node[n].get('orientation',[]) + [v]
	return
...
def checkOri(current,G,dicts,prev_orientations=None):
	...
	outs = []
	if G.node[current].get('orientation') is None: return dicts
	for d in dicts:
		if d[current] in G.node[current]['orientation']: outs.append(d)
...
class Scaffolder(object):
	...
	#
	def job(self,network,outputScaffolds,inpFastaRecords,readNet=False,distanceEstimation=False,threads=1):
		...
		self.cleaned_cover.writeScaffolds(outputScaffolds)
	...
	def greedyCover(self,G):
		...
		candidateEdges = OrderedDict((d['id'],(u,v)) 
				for u,v,d in 
					sorted(G.edges(data=True),key=lambda i: (i[2]['weight'],random()),reverse=False))
		...
		logging.debug(yellowText("Remaining edges/Connected components %s/%s" %(len(cover.edges()), len(sorted(nx.connected_components(cover))))))
		# return cover
		return cover,twins
	#
	def greedyCoverStrict(self,G,sorting="size"):
		...
		candidateEdges = OrderedDict((d['id'],(u,v)) 
				for u,v,d in 
					sorted(G.edges(data=True),key=lambda i: (i[2]['weight'] + factor,random()),reverse=False))
		for i in range(len(candidateEdges.values()) - 1): # this seems like a check... i would avoid it
			if cover.get_edge_data(*candidateEdges.values()[i])['weight'] < cover.get_edge_data(*candidateEdges.values()[i+1])['weight']: print 'Error!' # add logger here
		popped = set()
		while len(candidateEdges) > 0:
				...
				if cover.degree(n) > 1:
					for u_,v_,d_ in G.edges(n,data=True):
						if d_['id'] not in popped:
							...
							candidateEdges.pop(d_['id'])
							cover.remove_edge(u_,v_)
		print len(cover.edges()), len(sorted(nx.connected_components(cover)))
		# return cover
		return twins
	#
...
"""
	...
	def writeScaffoldsGen(self,out):
		...
		with open(report,"w") as out_handler:
			...
			for i,cc in enumerate(nx.connected_component_subgraphs(self)):
				for line in self.getConnectedComponentSequence(cc,"scaffold_%s" %i):
					if type(line) != Bio.SeqRecord.SeqRecord: out_handler.write(line)
				seqRecord = line
				if type(seqRecord) != Bio.SeqRecord.SeqRecord: print "!!!!"; pass # TODO add some exception here (should never reach it tho)
				yield seqRecord
	#
	def getNodeOri(self,n):
		...
		return ori
	#
	def getNodeSeq(self,n):
		""" obtain the sequence of a node """
		node = self.node[n]
		ori = node.get('orientation',[1])[-1]
		
		if ori == 1: return node['seq']
		else: return node['seq'].reverse_complement()
	#
	def getConnectedComponentSequence(self,cc,id_='scaffold'):
		...
		if len(cc) == 1:
			items = cc.nodes().iteritems().next()
			node,seq = items[0],items[1]['seq']
			start,end = 0,len(seq)
			...
			for i__,edge in enumerate(nx.dfs_edges(cc)):
				...
				if i == 0:
					...
					# 
					ids,starts,ends,i_s = ((node1,"gap",node2), (start,gapStart,start2),(end1,gapEnd,end), (1,2,3))
	...
	def tagEdgesToRemove(self,G):
		...
		for edge in  nx.dfs_edges(G_):
			...
			else: adjustNodes(G,ori_)

def tagEdgesToRemove(G,cc,**kwargs):
	""" traverse the cc from Graph G and remove edges making node orientation inconsistent """

	cc_ = cc.copy()
	for edge in  nx.dfs_edges(cc_):
		current,nxt = edge
		ori = oriToDict(cc[current][nxt]['orientation_max'],current) #
		ori_ = checkOri(current,cc,ori)
		if len(ori_) == 0:
			logging.debug("Edge %s has been tagged to be removed!" %str(edge))
			G.remove_edge(*edge)
		else: adjustNodes(cc,ori_)
